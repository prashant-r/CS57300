...
class DecisionTree(DTree):
    ...
    def _calculate_gini_gain(self, X, y, k):
        x_positive_count = np.sum(X[k]) + 1e-7
        x_negative_count = X.shape[1] - x_positive_count
        xy_pos_pos = np.dot(X[k], y)
        xy_pos_neg = x_positive_count - xy_pos_pos
        xy_neg_pos = np.dot( abs(X[k]-1), y )
        xy_neg_neg = x_negative_count - xy_neg_pos
        return 0 - ( (1 - (1.*xy_pos_pos / x_positive_count)**2 - (1.*xy_pos_neg / x_positive_count)**2)*x_positive_count +
         (1 - (1.*xy_neg_pos / x_negative_count)**2 - (1.*xy_neg_neg / x_negative_count)**2)*x_negative_count ) / X.shape[1]

    def _calculate_gini_gain_index(self, X, y):
        x_positive_count = np.sum(X, axis=1) + 1e-7
        x_negative_count = X.shape[1] - x_positive_count
        xy_pos_pos = np.dot(X, y)
        xy_pos_neg = x_positive_count - xy_pos_pos
        xy_neg_pos = np.dot( abs(X-1), y )
        xy_neg_neg = x_negative_count - xy_neg_pos
        return np.argmax( 0 - ( (1 - (1.*xy_pos_pos / x_positive_count)**2 - (1.*xy_pos_neg / x_positive_count)**2)*x_positive_count + 
            (1 - (1.*xy_neg_pos / x_negative_count)**2 - (1.*xy_neg_neg / x_negative_count)**2)*x_negative_count ) / X.shape[1] )
...
def kfoldCV(classifier, features, k, Assignment4 = False, seed = None):
    ...
    l.append(errors);
    l.append(mean);
    l.append(standardDeviation/math.sqrt(10));
